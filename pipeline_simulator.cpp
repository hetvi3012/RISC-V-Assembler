...
pair<vector<int>, vector<vector<int>>> stepPipelinedSimulator(KnobManager& knobs){
    ...
    if (!(stopFetching && isPipelineEmpty())) {
        ...
        if (knobs.printPipelineRegisters) {
            ...
            DualStreamBuf dualBuf(origBuf, snap.rdbuf());
            cout.rdbuf(&dualBuf);
        ...
        if (knobs.printRegistersEveryCycle) {
            ...
            DualStreamBuf dualBuf(origBuf, regSnap.rdbuf());
            cout.rdbuf(&dualBuf);
            printRegisterFile();
            std::cout.rdbuf(origBuf);
            logFile << "\n--- Register File ---\n" << regSnap.str();
...
int main(){
    ...
    if (!knob5File.is_open()) {
        std::cerr << "Error opening knob5_log.txt!\n";
    }

    std::string mcFilename = "output.mc";

    // Load program
    load_program_memory(mcFilename);

    // Ask user for knob inputs
    KnobManager knobs;
    knobs.readKnobInputs();

    char command;
    cout << "\nSimulation Control: (r)un, (s)tep, (c)lear, (q)uit\n";
    while (true) {
        cout << "Enter command: ";
        cin >> command;
        if (command == 'r'){
            runPipelinedSimulator(knobs);
        } else if (command == 's') {
            stepPipelinedSimulator(knobs);
        } else if (command == 'c') {
            resetPipelinedSimulator();
        } else if (command == 'q') {
            return 0;
        } else {
            cout << "Unknown command. Use r, s, c, or q." << endl;
        }
    }
